...
struct CoreContext {
  ...
  const Measurment &measure(std::string what) const { return _m[what]; }
...
template <typename PLATFORM> struct PlatformContext {
  PLATFORM *p;
  const PLATFORM *get_platform() const { return p; }
};

template <typename GRAPHICS> struct GraphicsContext {
  GRAPHICS *graphics;
  const GRAPHICS *get_graphics() const { return graphics; }
};
