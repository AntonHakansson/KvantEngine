...
public:
  static_assert(
      std::is_base_of<Kvant::blueprints::platform::Backend, PLATFORM>::value,
      "platform");
  using Platform = PLATFORM;

  static_assert(
      std::is_base_of<Kvant::blueprints::graphics::Backend<GRAPHICS<PLATFORM>>,
                      GRAPHICS<Platform>>::value,
      "platform");
  using Graphics = GRAPHICS<Platform>;

  template <typename ACTUAL_ASSET> using Asset = Kvant::Asset<ACTUAL_ASSET>;
  ...
  App() : _graphics(&_platform, &_platform._screen) {
    ...
    _platform.on_key_callback = [this](int k) {
      LOG_DEBUG << "WHATAFACK";
      this->on_key(k);
    };
  ...
  virtual ~App() { LOG_DEBUG << "App Cleanup."; }

  template <typename FIRST_SCENE> int run() {
...
private:
  ...
  void clear_frame() const { _graphics.clear_screen(); }
