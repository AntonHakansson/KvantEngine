...
namespace pos {
static glm::mat4 origin(int) { return glm::mat4(); }
...
template <int C = 1> class Transforms : public IsTransformable<Transforms<C>> {
public:
  template <typename CONTEXT>
  Transforms(const CONTEXT &ctx,
             std::function<glm::mat4(int)> pos_callback = [](int i) {
               UNUSED(i);
  ...
  glm::mat4 *begin() { return _transforms; }

  glm::mat4 *end() { return &_transforms[C]; }

  operator std::array<glm::mat4, C>() const { return *_transforms; }

  const glm::mat4 *get_transforms() const { return _transforms; }

  glm::mat4 *get_transforms_ptr(int i = 0) { return &_transforms[i]; }

  void set_transforms(std::function<glm::mat4(int index)> f, int begin = 0,
                      int end = C) {
  ...
  template <typename CONTEXT> void update(const CONTEXT &ctx) {
...
public:
  ...
  Instances(const CONTEXT &ctx,
            std::function<glm::mat4(int)> pos_callback =
                [](int i) {
                  UNUSED(i);
                  return glm::mat4();
                },
...
template <typename T> using Instance = Instances<T, 1>;

template <typename DRIVABLE> class no_driver {
public:
...
/**
 * Kvant::driven can extend any Kvant::is_transformable by allocating one or
 * more Kvant::driver objects to manipulate one or more 3D transformation.
 *
...
public:
  ...
  Driven(
      const CONTEXT &ctx,
      const std::function<typename DRIVER<typename T::TDrivable>::initializer(
          int)> &driver_callback = [](int i) ->
      typename DRIVER<typename T::TDrivable>::initializer {
        UNUSED(i);
        return typename DRIVER<typename T::TDrivable>::initializer();
      },
      const ARG &... args)
      : Container<T>(ctx, pos::origin, args...) {
    for (int j = 0; j < this->content()->size(); j++) {
      _drivers.push_back(std::make_unique<DRIVER<typename T::TDrivable>>(
  ...
  const T &drivable() const { return *this->ccontent(); }

  T *drivable_ptr() { return this->content(); }

  operator T() { return *this->content(); }
