...
template <typename GRAPHICS> struct Material {
...
public:
  Pipeline(std::string resource_base_name)
      : GRAPHICS::BasePipeline(resource_base_name) {}
...
public:
  ...
  const ForwardPipeline &draw(const SOMETHING &what) const {
    what.get_drawable().draw_instances(
        this->adhoc_context(), *this, what.get_transformable().get_transforms(),
        what.get_transformable().size());
    return *this;
  }

  template <typename CONTEXT, typename ENTITY>
  const ForwardPipeline &user_draw(const CONTEXT &ctx,
                                   const ENTITY &what) const {
    what.draw(ctx, *this);
    return *this;
  }
  ...
  template <typename PLUGABLE> auto &operator<<(const PLUGABLE &cc) const {
  ...
  void bind_instances() const { this->bind_instances_data(_itfm); }
  ...
  void enable_vertex_attributes() const { this->bind_vertex_attribs(); }
