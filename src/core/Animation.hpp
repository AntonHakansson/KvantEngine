...
struct Skeleton {
  ...
  int n_bones() const { return bones.size(); }
};
...
public:
  ...
  void update(const Kvant::CoreContext &ctx) { _animation_time += ctx.elapsed; }

  void reset() { _animation_time = 0; }

  static std::unique_ptr<Frame> interpolate(const Frame &f0, const Frame &f1,
                                            float amount) {
