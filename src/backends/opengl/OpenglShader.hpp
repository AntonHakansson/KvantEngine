...
namespace Kvant::graphics::opengl {
    ...
    class OpenglPipeline : public blueprints::graphics::Pipeline<GRAPHICS> {
        ...
        public:
            ...
            Uniform add_uniform(std::string id) const {
                GLint ret = GL_CHECK(glGetUniformLocation(this->shader_program, id.c_str()));
                if (ret == -1) LOG_WARNING << "Cannot find uniform named: " << id;
                return ret;
            }
            ...
            void bind_float_attrib(Attrib id, int count, int stride, void* ptr) const {
                GL_CHECK(glVertexAttribPointer(id, count, GL_FLOAT, GL_FALSE, sizeof(float) * stride, ptr));
    ...
    template <typename GRAPHICS, typename PLATFORM>
    void OpenglPipeline<GRAPHICS, PLATFORM>::load_shader(GLuint* shader, std::string buf, GLint shader_type) const {
    ...
    template <typename GRAPHICS, typename PLATFORM>
    void OpenglPipeline<GRAPHICS, PLATFORM>::use(const GraphicsContext<GRAPHICS>& ctx) const {
    ...
    template <typename GRAPHICS, typename PLATFORM>
    void OpenglPipeline<GRAPHICS, PLATFORM>::unuse(const GraphicsContext<GRAPHICS>& ctx) const {
