...
class Backend : public blueprints::graphics::Backend<Backend<PLATFORM>>,
                Screen::Subscriber {
public:
  ...
  Backend(PLATFORM *p, Screen *screen) { screen->subscribe(this); }
  ...
  void cull_on() const { glEnable(GL_CULL_FACE); }

  void cull_off() const { glDisable(GL_CULL_FACE); }
  ...
  void process_cmds(std::function<void(const SHADER &...)> cmds,
                    const SHADER &... s) {
    cmds(s...);
  }
