...
struct OpenglSurface
    : blueprints::graphics::Surface<GRAPHICS, OpenglSurface<GRAPHICS>> {
  GLuint vertex_vbo;
  GLuint triangle_vbo;
  ...
  OpenglSurface(const GraphicsContext<typename GRAPHICS::Backend> &ctx, Mesh *m)
      : Kvant::blueprints::graphics::Surface<GRAPHICS, OpenglSurface<GRAPHICS>>(
            ctx, m) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(uint32_t) * this->n_triangles * 3, &m->triangles[0],
                 GL_STATIC_DRAW);
  ...
  void draw_instanced(const GRAPHICS &backend, const PIPELINE &shader,
                      const glm::mat4 *transform, int count) const {
    GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, transform_vbo));
    shader.bind_instances();
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(glm::mat4) * count, transform,
                          GL_DYNAMIC_DRAW));
    ...
    GL_CHECK(glDrawElementsInstanced(GL_TRIANGLES, this->n_triangles * 3,
